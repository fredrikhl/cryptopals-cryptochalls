...
def encryption_oracle(s):
    ...
    s = pad(s)
    return cbcencrypt(s,KEY,KEY)

def decryption_oracle(s):
    plain = cbcdecrypt(s,KEY,KEY)
    if any(i > 0x80 for i in plain):
        raise ValueError("Invalid ASCII: " + plain.decode('latin'))
    return plain

s = encryption_oracle(b'A'*16)

s2 = s[:16] + bytes(16) + s[:16]
try:
    decryption_oracle(s2)
    print("No exception?")
except ValueError as e:
    p = str(e).split('ASCII: ')[1].encode('latin')
    k = xor(p[:16],p[32:48])
    assert k == KEY
